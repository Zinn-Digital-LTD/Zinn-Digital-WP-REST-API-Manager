...
Class UsersMeta extends MetaObject {
	...
	public function init()
	{
		...
		add_settings_section(
			'zinn_zwram_section_users',
			'', // Empty string is fine here, no need to translate
			function ()  {
				?>
				<p><?php esc_html_e('Select users metadata to include in REST API response', 'zinn-digital-wp-rest-api-manager'); ?></p>
		...
		foreach ($this->metaItemKeys as $metaItem) {
			add_settings_field(
				...
				function ($args) use($metaItem) {
					$options = get_option('zinn_zwram_options_user');
					?>
					<input type="checkbox" name="zinn_zwram_options_user[<?php echo esc_attr($args['label_for']); ?>]" value="1" <?php if (isset($options[$args['label_for']]) && $options[$args['label_for']] == 1) echo 'checked' ?> id="<?php echo esc_attr(ZINN_ZWRAM_FIELD_PREFIX . $metaItem); ?>">
					<?php
				},
				'zinn_zwram_user',
				'zinn_zwram_section_users',
